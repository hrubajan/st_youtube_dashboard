...
median_agg = df_agg_diff.iloc[:,numeric_cols].median()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_medians6mo = metric_medians6mo.iloc[1:]
    metric_medians12mo = metric_medians12mo.iloc[1:]

    # st.metric('Views',metric_medians6mo['Views'], 500)
    # st.write(metric_medians12mo)
    ...
    numeric_cols = np.array((df_agg_diff_final.dtypes == 'float64') | (df_agg_diff_final.dtypes == 'int64'))

    df_agg_numeric_lst = df_agg_diff_final.iloc[:,numeric_cols].median().index.tolist()
    df_to_pct = {}
